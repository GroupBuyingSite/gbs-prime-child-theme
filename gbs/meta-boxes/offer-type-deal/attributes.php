<?php do_action( 'gb_meta_box_item_attributes_pre' ) ?>
<?php do_action( 'gb_meta_box_attributes_pre' ) ?>
...
jQuery(document).ready( function($) {
	...
	$('a#gb_add_attribute').click( function() {
		...
		$('div#gb-deal-attributes table').show();
...
<?php
	$categories = SEC_Attribute::get_attribute_taxonomies(); ?>
<div id="gb-deal-attributes">
	<table class="widefat <?php if ( empty( $attributes ) ) echo 'cloak' ?>">
		<thead>
			<tr>
				...
				<?php do_action( 'gb_meta_box_attributes_rows_header' ) ?>
			...
			<?php foreach ( $attributes as $post_id => $data ): ?>
				...
				<tr class="gb-deal-attributes-row">
					<td class="title">
						...
						<input disabled="disabled" class="gb-deal-attribute" type="text" size="15" value="<?php esc_attr_e( $data['title'] ); ?>" name="gb-attribute[title][]" placeholder="<?php gb_e( 'Title' ); ?>" />
					</td>
					<td class="price"><input class="gb-deal-attribute" type="text" size="10" placeholder="<?php gb_e( 'Default' ); ?>" value="<?php if ( $data['price']!=SEC_Attribute::DEFAULT_PRICE ) {esc_attr_e( $data['price'] );} ?>" name="gb-attribute[price][]" /></td>
					...
					<td class="category">
						...
						<?php foreach ( SEC_Attribute::get_attribute_taxonomies() as $taxonomy ): ?>
							<tr>
					...
					<?php do_action( 'gb_meta_box_attributes_rows', $post_id, $data ) ?>
...
<table id="gb-deal-attributes-template" style="display: none;"><tbody>
	<tr class="gb-deal-attributes-row">
		<td class="title">
			<input class="gb-deal-attribute hidden" type="hidden" value="0" name="gb-attribute[attribute_id][]" /><input class="gb-deal-attribute" type="text" size="15" value="" name="gb-attribute[title][]" placeholder="<?php gb_e( 'Title' ); ?>" /><span class="howto"><small><?php gb_e( '(Permanent & Required)' ) ?></span></p></td>
		<td class="price"><input class="gb-deal-attribute" type="text" size="10" value="" name="gb-attribute[price][]" placeholder="<?php gb_e( 'Default' ); ?>" /><span class="howto"><small><?php gb_e( '(Leave blank to use Offer price)' ) ?></small></span></td>
		...
		<?php endif ?>
		<?php do_action( 'gb_meta_box_attributes_rows_js' ) ?>
...
<?php do_action( 'gb_meta_box_item_attributes' ) ?>
<?php do_action( 'gb_meta_box_attributes' ) ?>
