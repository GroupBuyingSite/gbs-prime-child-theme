...
if ( class_exists( 'Group_Buying_Theme_UI' ) ) {
	...
	include get_template_directory() . '/gbs-addons/facebook/template-tags.php';
	...
	class Group_Buying_Facebook_Connect extends Group_Buying_Theme_UI {
		...
		public static function init() {
			...
			if ( self::$fb_app_id != '' || self::$fb_app_id != '' ) {
				...
				add_filter( 'gb_logout_url' , array( get_class(), 'log_out_url' ), 20, 2 );
...
?>
		...
		public static function log_out_url() {
			if ( self::is_facebook_logged_in() != false ) {
				$link = '<a onclick="logoutFacebookUser()" href="javascript:void()" title="'.gb__( 'Logout' ).'" class="logout">'.gb__( 'Logout' ).'</a>';
			} else {
				$link = '<a href="' . gb_get_logout_url() . '" title="'.gb__( 'Logout' ).'" class="logout">'.gb__( 'Logout' ).'</a>';
			}
			echo apply_filters( 'gb_facebook_log_out_url', $link );
		}
		...
		public static function button( $button_text = 'Login with Facebook' ) {
			...
			if ( !empty( self::$fb_app_id ) || !empty( self::$fb_app_id ) ) {
		...
		public static function get_facebook_authorization_request() {
			...
			if ( !is_null( $signed_request ) ) {
				...
				if ( $access_token ) {
					// Store access token
					self::store_facebook_access_token( $access_token, $expires );
					...
					gb_set_message( gb__('<strong>Facebook Connect Error:</strong> ') . $errors->error->message, Group_Buying_Controller::MESSAGE_STATUS_ERROR );
				}
		...
		public static function store_facebook_access_token( $token, $expiration ) {
